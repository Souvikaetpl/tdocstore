...
## 12. Authentication & user accounts (new in v6, decided in v7)
...
**What this now concretely requires, not yet built:**
- A `users` table (stable internal ID, email, display name, created-at) and an `identities`/`linked_accounts` table (`user_id`, `provider` = `"google"`, `provider_user_id`) — kept separate from the start specifically so method 2 above holds.
- A `Sign in with Google` OAuth flow (Google Cloud OAuth client credentials, the redirect/callback handling, session issuance after a successful callback).
- A session mechanism (cookie or token) for the website itself, separate from the personal MCP tokens in item 3.
- A per-user MCP token: generated in account settings, stored (hashed, like a password) against the user row, checked by the MCP server on every tool call (§13 item 1).
- Per-user tables for whatever personalization ships first (saved searches and/or bookmarks are the natural starting set) — foreign-keyed to `users.id`.

**What doesn't change regardless of the above**: this reopens the Auth question from §3, but not the Storage/Kong/Studio parts of that reasoning — self-hosted Supabase's *Auth* module (GoTrue) is a legitimate option to evaluate for the OAuth/session mechanism above, against building it directly on the existing Postgres (e.g. via a lightweight library) or a third-party auth provider (Auth0, Clerk, or similar); adopting Supabase wholesale for Storage/Kong/Studio is still not justified by this alone (§3 unchanged on that point).

**Real security risk this section is honestly not yet mitigating**: session/token design, and rate-limiting on the OAuth callback and MCP-token-generation endpoints specifically, aren't built yet. See §15 for this recorded as an explicit risk.

**5. Admin-issued test accounts, alongside Google sign-in (added after v7).** The project owner needs to be able to hand testers working access without them going through Google sign-in, and to cut that access off on demand. This is additive to the identity model above, not a change to it — a test account is just a `users` row with no Google identity attached, created directly instead of via self-service signup:
- `users.is_admin` (boolean): marks the project owner's own account as able to create/disable test accounts. One flag is enough while there is exactly one admin — a full roles/permissions system isn't justified yet.
- `users.status` (`active` / `disabled`): the on/off switch for any account, test or Google-linked. Set by the admin, not the user.
- **How a test account logs in, without Google — still an open choice, not yet decided:** either the admin sets a username + password directly on the account (`provider="local"` identity row, convenient for a tester logging in repeatedly over several days), or the admin generates a one-time login link/token that establishes a session immediately (nothing reusable to leak, easier to revoke outright, less convenient for repeat logins). Pick this when the test-account feature is actually built, not now.
- **Revocation must be checked on every request, not only at login.** A session that's just a signed cookie/token with an expiry can't be killed mid-flight — disabling the account would only block the *next* login, leaving an already-logged-in tester working until their session naturally expires. To make "revoke" actually immediate, each authenticated request re-checks `users.status` (e.g. a lookup keyed by session/user ID, cheap enough to do per-request against Postgres directly, or cached briefly if it becomes a bottleneck) — not just something checked once at sign-in time.
